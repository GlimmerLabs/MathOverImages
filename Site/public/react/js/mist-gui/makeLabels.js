...
export function init(actionArray, builtinFunctions) {
  ...
  function getInfoText(key) {
    var info = builtinFunctions.get(key);
    return key + '(' + info.params + ')\n\n' + info.about;
  };

  const menuFunctionDescriptions = {}

  const funKeys = builtinFunctions.keys();
  for(var i = 0; i < funKeys.length; i++) {
    const key = funKeys[i];
    menuFunctionDescriptions[key] = getInfoText(key);
  }

  const menuValuesDescriptions = {
  ...
  const pointerWidth = size.width / 90;
  const pointerHeight = size.width / 104;
  const triRadius = size.width / 156;
  ...
  return {
    makeLabel(group) {
      ...
      if (rep == 'rgb') {
        ...
        var rgbText = new Kinetic.Text({
          text: text,
          x: -110,
          y: pointerHeight + (1.1 * functionStyle.totalSideLength),
          padding: 5,
          width: 160,
          fill: 'black'
        });
    ...
    */
        ...
        var label = new Kinetic.Label ({
          x: group.x() + (functionStyle.totalSideLength / 2) + xOffset,
          y: group.y() + (1.1 * functionStyle.totalSideLength),
        });
    ...
    makeToolLabel(group, toolboxGroup) {
