...
export default function(
  ...
  layers.drag.on('mouseup', function(evt) {
    ...
    if (state.scaledObj) {
      ...
      replaceNode(state.scaledObj, group);
  ...
  layers.drag.on('dragmove', function() {
    if (state.dragShape != null) {
      ...
      if (node) {
        ...
        if ((isValue(group) && isValue(state.dragShape)) ||
          (isFunction(group) && isFunction(state.dragShape))) {
          group.setAttrs({
            scaleX: 1.2,
            scaleY: 1.2
          });
