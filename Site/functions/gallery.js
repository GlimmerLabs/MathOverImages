...
module.exports.getRandomImages= (function(count, callback) {
  ...
  filedatabase.query("SELECT * FROM images ORDER BY RAND() LIMIT " + count, function(rows, error){
    callback(rows, error);
  });
...
module.exports.getRecentImages= (function(count, callback) {
  ...
  filedatabase.query("SELECT * FROM images ORDER BY modifiedAt DESC LIMIT " + count, function(rows, error){
    callback(rows, error);
  });
